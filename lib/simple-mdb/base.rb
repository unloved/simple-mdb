  ...
  class Base
    ...
    class << self # Class methods
      ...
      def find_where(conditions)
        SimpleMdb::MDBTools.sql_select_where(mdb_file, table_name, nil, conditions).collect! { |record| instantiate(record) }
      end
      ...
      def conditions_from_hash(hash)
        ...
        SimpleMdb::MDBTools.compile_conditions(hash) do |column_name, value|
      ...
      def instantiate(record)
        ...
        record.each do |name,value|
          # begin
            new_hash[SimpleMdb::MDBTools.methodize(name)] = column_for_field(name).type_cast(value)
          # rescue
    ...
    def method_missing(method_id, *args, &block)
       method_name = SimpleMdb::MDBTools.methodize(method_id.to_s)
